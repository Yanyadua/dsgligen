...
EXPERIMENT_NAME="${EXPERIMENT_NAME:-vg_fixedsplit_quality_guard_300_20260620}"
TOTAL_ITERS="${TOTAL_ITERS:-300}"
SAVE_EVERY_ITERS="${SAVE_EVERY_ITERS:-100}"
ALLOW_RESUME="${ALLOW_RESUME:-0}"
OUTPUT_ROOT="${OUTPUT_ROOT:-/root/autodl-tmp/GLIGEN/OUTPUT_FIXED_QUALITY_GUARD}"
...
if find "$EXPERIMENT_ROOT" -name checkpoint_latest.pth -print -quit 2>/dev/null | grep -q .; then
  if [[ "$ALLOW_RESUME" != "1" ]]; then
    ...
    echo "Set ALLOW_RESUME=1 only for an intentional continuation."
    exit 2
  fi
fi
...
/root/miniconda3/bin/python scripts/eval/audit_historical_vg_run.py \
  --config configs/vg_fixedsplit_scene_graph_quality_guard.yaml \
...
exec /root/miniconda3/bin/python main.py \
  --yaml_file configs/vg_fixedsplit_scene_graph_quality_guard.yaml \
