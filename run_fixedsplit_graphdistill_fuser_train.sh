...
SAVE_EVERY_ITERS="${SAVE_EVERY_ITERS:-250}"
ALLOW_RESUME="${ALLOW_RESUME:-0}"
OUTPUT_ROOT="${OUTPUT_ROOT:-/root/autodl-tmp/GLIGEN/OUTPUT_FIXED_GRAPHDISTILL_FUSER}"
...
if find "$EXPERIMENT_ROOT" -name checkpoint_latest.pth -print -quit 2>/dev/null | grep -q .; then
  if [ "$ALLOW_RESUME" != "1" ]; then
    ...
    echo "Set ALLOW_RESUME=1 only when this continuation is intentional."
    exit 2
  fi
fi
...
/root/miniconda3/bin/python scripts/eval/audit_historical_vg_run.py \
  --config configs/vg_fixedsplit_scene_graph_graphdistill_fuser.yaml \
...
exec /root/miniconda3/bin/python main.py \
  --yaml_file configs/vg_fixedsplit_scene_graph_graphdistill_fuser.yaml \
