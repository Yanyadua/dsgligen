...
# Fixed-ten diagnostic only; no FID/IS/OOR is produced here.
...
run_one() {
  local policy="$1"
  export CAPTION_POLICY="$policy"
  export OUT_DIR="/root/autodl-tmp/GLIGEN/eval_outputs/clean10_triplet_fuser500_${policy}_c_no_token_20260713"
  if [[ -e "$OUT_DIR" ]]; then
