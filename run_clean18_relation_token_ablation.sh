...
run_one() {
  ...
  export MAX_RELATION_GROUNDING_TOKENS="$max_tokens"
  export RELATION_GROUNDING_TEMPLATE="$template"
  export OUT_DIR="/root/autodl-tmp/GLIGEN/eval_outputs/${name}"

  rm -rf "$OUT_DIR"
  echo "===== START $name max_tokens=$max_tokens template=$template $(date +%F\ %T) ====="
  /root/miniconda3/bin/python scripts/eval/generate_vg_fixedsplit_eval.py
  echo "===== DONE $name $(date +%F\ %T) ====="
}

run_one \
  clean18_relabl_plain1_ddim50_20260708 \
  1 \
  "{subject} {predicate} {object}"

run_one \
  clean18_relabl_plain2_ddim50_20260708 \
  2 \
  "{subject} {predicate} {object}"

run_one \
  clean18_relabl_plain3_ddim50_20260708 \
  3 \
  "{subject} {predicate} {object}"

run_one \
  clean18_relabl_spatial3_ddim50_20260708 \
  3 \
